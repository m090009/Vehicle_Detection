...
def combined_color_threshold(img, red_thresh, hls_thresh):
    ...
    binary_threshold[(red_binary_threshold == 1) | (hls_binary_threshold == 1)] = 1
...
def get_rad_curv(y_vals, x_vals):
...
class LaneLineFinder:
    ...
    def process_image(self, image):
        ...
        if not self.keras_model:
            ...
            # window settings
            window_width = 50
            window_height = height / 5  # Break image into 5 vertical layers since image height is 720
            ...
            if len(leftx) > 0 and len(lefty) > 0 and len(rightx) > 0 and len(righty) > 0:
                ...
                # Get car position
                car_position = get_vehicle_position()
                # frame_to_return = self.average_frame_sampling()
                # Draw Drivable area
                ...
                write_text_on_image(masked_lane_image, int(lane_curvature))
    ...
    def average_lane_sampling(self, line_fit, previous_fits):
        ...
        if len(previous_fits) > 0:
            ...
            # print(line_fit.shape)
            # exit()
            # line_fit = tuple(map(tuple, line_fit))
...
def write_text_on_image(img, text):
    ...
    bottomLeftCornerOfText = (int(img.shape[1] * 0.36), int(img.shape[0] * 0.10))
    fontScale              = 1
    fontColor              = (255,255,255)
    lineType               = 2
    img[:100, :] -= 5 
    curv = 'Straigh' if text > 3000 else text
    cv2.putText(img, 'Curvature is ({}) meters'.format(text), 
